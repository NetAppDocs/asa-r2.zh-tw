...
searchtitle: ASA r2 與其他 ONTAP 系統的差異 
---
= ASA r2 與其他 ONTAP 系統的差異
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA r2 系統為基於全快閃解決方案的純 SAN 環境提供硬體和軟體解決方案。r2 系統與其他ONTAP系統（ ASA 、 AFF和FAS）在ONTAP特性、儲存層和支援協定的實作方面有所不同。
...
建立儲存單元（基於 LUN 或 NVMe 命名空間）時，ONTAP 會自動在對應的儲存可用區中建立一個磁碟區來存放該儲存單元。新建立的磁碟區會自動放置在儲存可用區內，以達到最佳效能和均衡的容量利用率。儲存可用區內的容量利用率均衡取決於您的 ONTAP 版本。link:../administer/space-balancing.html["了解ASA r2 叢集中的容量均衡"^]。
...
.使用協定服務建立預設儲存虛擬機
新叢集會自動包含一個預設資料儲存虛擬機器 (VM)，並啟用 SAN 協定。  IP 資料 LIF 支援 iSCSI 和 NVMe/TCP 協議，並使用 `default-data-blocks`預設服務策略。
