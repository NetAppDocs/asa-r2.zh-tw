...
在 ONTAP 9.18.1 及更早版本中，同時包含 iSCSI 和 NVMe/TCP 流量的 IP LIF 不支援自動 LIF 容錯移轉。建立僅包含 iSCSI 流量的 LIF 時，預設會啟用自動 LIF 容錯移轉，除非儲存設備 VM 上已存在未啟用自動 LIF 容錯移轉的 iSCSI LIF。如果儲存設備 VM 上存在未啟用自動 LIF 容錯移轉的 iSCSI LIF，則新建立的 LIF 也不會啟用自動 LIF 容錯移轉。在這種情況下，您必須先刪除現有的 iSCSI LIF，然後重新建立並啟用自動 LIF 容錯移轉。了解如何link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["在 SAN 環境中刪除 LIF"^]。
