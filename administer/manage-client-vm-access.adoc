...
如果某個元件發生故障，LIF 可以容錯移轉或移轉至不同的實體連接埠，從而避免網路通訊中斷。從 ONTAP 9.19.1 開始，預設支援 iSCSI、NVMe/TCP 和共享的 IP LIF 的自動 LIF 容錯移轉。啟用自動 LIF 容錯移轉後，如果發生儲存設備容錯移轉，LIF 將自動從其主節點或連接埠移轉至其 HA 合作夥伴節點或連接埠，並在容錯移轉完成後移轉回來。或者，如果連接埠發生故障，LIF 將自動移轉至其目前主節點中狀態正常的連接埠，並在該連接埠恢復正常後移轉回原始連接埠。

在 ONTAP 9.18.1 及更早版本中，NVMe/TCP LIF 或共享的 IP LIF 不支援自動 LIF 容錯移轉。建立僅 iSCSI 的 LIF 時，預設會啟用自動 LIF 容錯移轉，除非儲存設備虛擬機器上已存在未啟用自動 LIF 容錯移轉的 iSCSI LIF。如果儲存設備虛擬機器上存在未啟用自動 LIF 容錯移轉的僅 iSCSI LIF，則新建立的 LIF 也不會啟用自動 LIF 容錯移轉。在這種情況下，您必須先刪除現有的僅 iSCSI LIF，然後重新建立並啟用自動 LIF 容錯移轉。了解如何link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["在 SAN 環境中刪除 LIF"^]。

持久性連接埠用於 FC 和 NVMe/FC LIF 的容錯移轉。深入瞭解 ONTAP 文件中的 link:https://docs.netapp.com/us-en/ontap/san-config/asa-support-limitations.html#support-for-persistent-ports["持續連接埠"^]。
...
在 ONTAP 9.18.1 及更早版本中，如果儲存 VM 上的所有其他 iSCSI LIF 也啟用了自動 LIF 容錯移轉，則建立僅 iSCSI LIF 時預設啟用自動 LIF 容錯移轉。如果儲存 VM 上存在未啟用自動 LIF 容錯移轉的僅 iSCSI LIF，則新建立的 LIF 也不會啟用自動 LIF 容錯移轉。
...
.相關資訊
* link:https://docs.netapp.com/us-en/ontap/san-admin/move-san-lifs-task.html["移動 SAN LIF"]。
* link:https://docs.netapp.com/us-en/ontap/san-admin/remove-lif-from-portset-task.html["從 PortSet 移除 SAN LIF"]。
