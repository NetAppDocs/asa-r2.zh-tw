...
在向節點或機架安裝新驅動器之前，請使用NetAppHardware Universe來確認要新增的驅動器是否受ASA r2 系統支持，並確定新驅動器的正確插槽。根據系統型號和ONTAP版本不同，新增磁碟機的正確插槽也會有所不同。在某些情況下，您需要按順序將硬碟機新增至指定的插槽。
