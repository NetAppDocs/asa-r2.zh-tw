...
如果您需要從機載金鑰管理程式切換至外部金鑰管理程式，或是從外部金鑰管理程式切換至外部金鑰管理程式link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/migrate-keys-between-key-managers.html["在金鑰管理員之間移轉 ONTAP 資料加密金鑰"^]，請參閱。
