...
summary: 啟用 ARP 和 AI 來偵測潛在的勒索軟體威脅、自動採取措施保護您的資料，並在 System Manager 中顯示警告訊息，提醒您注意可疑活動。 
---
= 在您的 ASA r2 儲存系統上啟用基於 AI 的自主勒索軟體防護
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ARP 透過採用機器學習模式進行反勒索軟體分析，進而提升網路彈性，該模式能夠偵測 SAN 環境中不斷演變的勒索軟體。ARP 的機器學習模型在模擬勒索軟體攻擊前後，均使用大型檔案資料集進行預訓練。這種資源密集型訓練在 ONTAP 外部進行，訓練產生的預訓練模型已整合到 ONTAP 系統中。該模型不可存取或修改。ARP/AI 在啟用後立即生效；無需link:https://docs.netapp.com/us-en/ontap/anti-ransomware/index.html#learning-and-active-modes["學習期"^]。
...
* 從 ONTAP 9.19.1 RC 版本開始，基於人工智慧的自主勒索軟體防護（ARP/AI）支援 ASA r2 系統上處於 SnapMirror 活動同步 SAN 關係中的主讀寫儲存單元。ARP/AI 在活動主儲存單元上運行，建立僅針對主儲存單元的 ARP 快照，並為這些儲存單元提供基於機器學習的勒索軟體偵測。
