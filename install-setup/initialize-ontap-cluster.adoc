...
在某些情況下，你可能需要 link:../learn-more/cli-support.html#set-up-an-ontap-asa-r2-cluster-using-the-cli["使用ONTAP命令列介面 (CLI) 設定集群"]。例如，如果您的安全協定不允許您將筆記型電腦連接到管理交換機，或者如果您使用的是非 Windows 作業系統，則應使用 CLI。
