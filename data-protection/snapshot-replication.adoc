...
為了增強抵禦勒索軟體攻擊的安全性，請建立 SnapLock for SnapVault 複寫關係。
...
透過將快照複寫到遠端叢集來保護您的資料。一致性群組的複寫快照可用於災難恢復或資料移轉。
...
.. 搜尋並選取來源一致性群組。
.. 在「複寫目的地」下，選擇要複寫到的叢集，然後選擇儲存設備虛擬機器。


|===
. 選取複寫原則。
. 若要延遲資料傳輸的開始，請選擇 *Destination settings*，然後取消選擇 *Start transfer immediately*。
+
根據預設、初始資料傳輸會立即開始。

. （可選）若要置換預設傳輸排程，請選擇 *Destination settings*，然後選擇 *Override transfer schedule*。
+
...
.接下來呢？
建立複寫原則和關係後，初始資料傳輸將按照複寫原則中的定義開始。您可以選擇測試複寫容錯移轉，以驗證在 ASA r2 系統離線時是否能成功執行容錯移轉。



[[create-a-relationship-to-replicate-snapshots-with-snaplock-for-snapvault]]
=== 建立關係以使用 SnapLock for SnapVault 複寫快照

為了防範勒索軟體攻擊，在設定複寫關係時，您可以選擇保存庫複寫原則，然後選擇鎖定目的地快照，藉此啟用 SnapLock for SnapVault。受 SnapLock for SnapVault 保護的快照不會遭到意外或惡意刪除。


NOTE: ASA r2 系統不支援 SnapLock Compliance 模式或 SnapLock Enterprise 模式。建立 SnapLock for SnapVault 複寫關係僅能鎖定目的地快照。並不能啟用 SnapLock Compliance 或 Enterprise 模式。

.開始之前
您必須 link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock["初始化 SnapLock Compliance 時鐘"]。

.步驟
. 在 System Manager 中，選擇 *Protection > Consistency groups* ，然後選擇一致性群組。
. 選擇 image:icon_kabob.gif["三個垂直藍點"]，然後選擇 *保護*。
. 在 * 遠端保護 * 下、選取 * 複寫到遠端叢集 * 。
. 選取 * 複寫原則 * 。
+
您必須選取 _Vault_ 複寫原則。

. 選擇「目的地設定」，然後選擇「鎖定目的地快照以防止刪除」。
. 輸入資料保留期間上限和下限。
...
.接下來呢？
建立複寫關係後，初始資料傳輸將按照複寫原則中的定義開始。您可以選擇測試複寫容錯移轉，以驗證在 ASA r2 系統離線時是否能成功進行容錯移轉。
