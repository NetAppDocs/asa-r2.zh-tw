...
NOTE: 從 ONTAP 9.19.1 RC 版本開始，人工智慧自主勒索軟體防護（ARP/AI）支援 ASA r2 系統上處於 SnapMirror 活動同步 SAN 關係中的主讀寫儲存單元。ARP/AI 在活動主儲存單元上運行，建立僅針對主儲存單元的 ARP 快照，並為這些儲存單元提供基於機器學習的勒索軟體偵測。若要啟用 ARP/AI，請參閱link:../secure-data/enable-anti-ransomware-protection.html["利用 AI 啟用自主勒索軟體保護"]。
