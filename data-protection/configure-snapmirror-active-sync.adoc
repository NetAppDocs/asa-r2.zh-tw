...
.關於這項工作
如果您使用 iSCSI 或 FC 協定並使用適用於 VMware Sphere 的ONTAP工具，則可以選擇link:https://docs.netapp.com/us-en/netapp-solutions/vmware/vmware-vmsc-with-smas.html["使用ONTAP Tools for VM ware 設定SnapMirror主動同步"^] 。


NOTE: 從 ONTAP 9.19.1 開始，具備人工智慧的自主勒索軟體防護 （ARP/AI） 支援 ASA r2 系統上 SnapMirror 主動同步 SAN 關係中的主讀寫儲存設備單元。ARP/AI 在作用中的主儲存設備單元上執行，建立僅限主儲存設備的 ARP 快照，並為這些儲存設備單元提供以機器學習為基礎的勒索軟體偵測。若要啟用 ARP/AI，請參閱 link:../secure-data/enable-anti-ransomware-protection.html["利用 AI 啟用自主勒索軟體保護"]。
