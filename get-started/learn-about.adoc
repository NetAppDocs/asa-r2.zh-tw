...
summary: ASA R2 系統（ ASA A1K ， A90 ， A70 ）提供統一化的硬體與軟體解決方案，為僅限 SAN 的客戶提供簡化的體驗。 
searchtitle: 瞭解 ASA A1K ， A90 和 A70 系統 
---
= 瞭解 ASA R2 儲存系統
:allow-uri-read: 
:icons: font
...
ASA R2 系統支援所有 SAN 傳輸協定（ iSCSI ， FC ， NVMe / FC ， NVMe / TCP ）。iSCSI ， FC 和 NVMe / FC 傳輸協定支援對稱的雙主動式多重路徑架構，因此主機和儲存設備之間的所有路徑都是主動 / 最佳化的。iSCSI 和 NVMe / TCP 傳輸協定支援主機和儲存設備之間的直接路徑。
