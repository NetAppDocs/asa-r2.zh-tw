...
ASA r2 系統中的每個 HA 配對都使用名為_儲存可用性區域_的通用儲存資源池。儲存可用性區域可存取儲存系統中所有可用的磁碟，且 HA 配對中的兩個節點均可看到。監控儲存可用性區域的可用空間可讓您視需要減少使用量或增加容量，以免因空間不足而導致工作負載失敗。

如果叢集中有 4 個以上的節點，您可以檢視每個 HA 配對的儲存可用性區域所使用的空間量。此度量不適用於雙節點叢集。

從 ONTAP 9.19.1 開始,如果您的 ASA r2 系統 HA 配對中的某個儲存可用區接近容量上限,System Manager *Insights* 中將顯示警示。深入瞭解 System Manager link:view-insights.html["深入分析"]。
...
+
在 *Storage units* 下，會顯示儲存可用區中所有儲存單元的清單。



.接下來呢？
如果您的儲存可用區空間不足，您應該計劃link:../manage-data/move-storage-units.html["移動儲存單元"]到另一個儲存可用區，以平衡叢集的儲存利用率、刪除快照以釋放空間，或向節點新增額外的磁碟以增加容量。
